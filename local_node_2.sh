...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	$DNOM config set client chain-id "$CHAINID" --home "$HOMEDIR"
	$DNOM config set client keyring-backend "$KEYRING" --home "$HOMEDIR"
	...
	echo "$USER1_MNEMONIC" | $DNOM keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER2_MNEMONIC" | $DNOM keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER3_MNEMONIC" | $DNOM keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER4_MNEMONIC" | $DNOM keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	...
	# jq -r --arg moderator_address "$($DNOM keys show dev2 --address --keyring-backend "$KEYRING" --home "$HOMEDIR")" '.app_state["distribution"]["moderator_address"] = $moderator_address' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	# jq -r --arg base_address "$($DNOM keys show dev3 --address --keyring-backend "$KEYRING" --home "$HOMEDIR")" '.app_state["distribution"]["base_address"] = $base_address' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	$DNOM genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000aguru --keyring-backend "$KEYRING" --home "$HOMEDIR"
	$DNOM genesis add-genesis-account "$USER1_KEY" 1000000000000000000000aguru --keyring-backend "$KEYRING" --home "$HOMEDIR"
	$DNOM genesis add-genesis-account "$USER2_KEY" 1000000000000000000000aguru --keyring-backend "$KEYRING" --home "$HOMEDIR"
	$DNOM genesis add-genesis-account "$USER3_KEY" 1000000000000000000000aguru --keyring-backend "$KEYRING" --home "$HOMEDIR"
	$DNOM genesis add-genesis-account "$USER4_KEY" 1000000000000000000000aguru --keyring-backend "$KEYRING" --home "$HOMEDIR"

	# Sign genesis transaction
	$DNOM genesis gentx "$VAL_KEY" 1000000000000000000000aguru --gas-prices ${BASEFEE}aguru --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
	...
	# Collect genesis tx
	$DNOM genesis collect-gentxs --home "$HOMEDIR"

	# Run this to ensure everything worked and that the genesis file is setup correctly
	$DNOM genesis validate-genesis --home "$HOMEDIR"
