...
func (md MonoDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (newCtx sdk.Context, err error) {
	...
	for i, msg := range tx.GetMsgs() {
		ethMsg, err := evmtypes.UnpackEthMsg(msg)
		if err != nil {
			return ctx, err
		}

		ethTx := ethMsg.AsTransaction()
		feeAmt := evmtypes.EffectiveFee(ethTx, nil)
		gas := ethTx.Gas()
		...
		if ethTx.Type() == ethtypes.DynamicFeeTxType && decUtils.BaseFee != nil {
			...
			feeAmt = evmtypes.EffectiveFee(ethTx, decUtils.BaseFee)
			fee = sdkmath.LegacyNewDecFromBigInt(feeAmt)
		}
		...
		// current message.
		decUtils.TxFee.Add(decUtils.TxFee, evmtypes.EffectiveFee(ethTx, nil))
