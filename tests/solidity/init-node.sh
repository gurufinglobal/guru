...
CHAINDIR="$HOME/.tmp-gurud-solidity-tests" # TODO: make configurable like chain id
...
# Set client config
gurud config set client chain-id "$CHAINID" --home "$CHAINDIR"
gurud config set client keyring-backend "$KEYRING" --home "$CHAINDIR"
...
echo "$VAL_MNEMONIC" | gurud keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
echo "$USER1_MNEMONIC" | gurud keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
echo "$USER2_MNEMONIC" | gurud keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
echo "$USER3_MNEMONIC" | gurud keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
echo "$USER4_MNEMONIC" | gurud keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"

# Set moniker and chain-id for Cosmos EVM (Moniker can be anything, chain-id must be an integer)
gurud init "$MONIKER" --chain-id "$CHAINID" --home "$CHAINDIR"

# Change parameter token denominations to agxn
...
gurud genesis add-genesis-account "$(gurud keys show "$VAL_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 100000000000000000000000000agxn --keyring-backend "$KEYRING" --home "$CHAINDIR"
gurud genesis add-genesis-account "$(gurud keys show "$USER1_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000agxn --keyring-backend "$KEYRING" --home "$CHAINDIR"
gurud genesis add-genesis-account "$(gurud keys show "$USER2_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000agxn --keyring-backend "$KEYRING" --home "$CHAINDIR"
gurud genesis add-genesis-account "$(gurud keys show "$USER3_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000agxn --keyring-backend "$KEYRING" --home "$CHAINDIR"
gurud genesis add-genesis-account "$(gurud keys show "$USER4_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000agxn --keyring-backend "$KEYRING" --home "$CHAINDIR"
...
gurud genesis gentx "$VAL_KEY" 1000000000000000000000agxn --gas-prices ${BASEFEE}agxn --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
...
# Collect genesis tx
gurud genesis collect-gentxs --home "$CHAINDIR"

# Run this to ensure everything worked and that the genesis file is setup correctly
gurud genesis validate-genesis --home "$CHAINDIR"

# Start the node
gurud start "$TRACE" \
	--log_level $LOGLEVEL \
	--minimum-gas-prices=0.0001ugxn \
