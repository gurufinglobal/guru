...
func TestNodeSourceDoesNotImportPublicPulsarAPI(t *testing.T) {
	repoRoot := projectRootFromTestFile(t)
	...
	if len(offenders) > 0 {
		t.Fatalf("node source must use internal gogo types, found public Pulsar imports in: %s", strings.Join(offenders, ", "))
	}
}
