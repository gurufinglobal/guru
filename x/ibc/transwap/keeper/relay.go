...
func (k Keeper) DenomPathFromHash(ctx sdk.Context, ibcDenom string) (string, error) {
	...
	return types.DenomPath(denom), nil
}

// createPacketDataBytesFromVersion creates the packet data bytes to be sent based on the application version.
// func createPacketDataBytesFromVersion(appVersion, sender, receiver, memo string, token transwapv1.Token) ([]byte, error) {
// 	switch appVersion {
// 	case types.V1:
// 		packetData := types.NewFungibleTokenPacketData(types.DenomPath(token.Denom), token.Amount, sender, receiver, memo)

// 		if err := types.ValidateFungibleTokenPacketData(packetData); err != nil {
// 			return nil, errorsmod.Wrapf(err, "failed to validate %s packet data", types.V1)
// 		}

// 		return types.FungibleTokenPacketDataBytes(packetData), nil
// 	default:
