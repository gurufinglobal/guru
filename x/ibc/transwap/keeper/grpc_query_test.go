...
func TestKeeperDenomQuery(t *testing.T) {
	...
	t.Run("invalid hash returns error", func(t *testing.T) {
		...
		_, err := k.Denom(ctx, &types.QueryDenomRequest{Hash: "ibc/not-a-hex"})
		require.Error(t, err)
		require.Equal(t, codes.InvalidArgument, status.Code(err))
	})
	...
	t.Run("missing denom returns not found", func(t *testing.T) {
		...
		_, err := k.Denom(ctx, &types.QueryDenomRequest{Hash: "ibc/" + missingHash})
		require.Error(t, err)
	...
	t.Run("found denom", func(t *testing.T) {
		...
		resp, err := k.Denom(ctx, &types.QueryDenomRequest{Hash: "ibc/" + types.DenomHash(denom).String()})
...
func TestKeeperDenomsQuery(t *testing.T) {
	...
	secondPage, err := k.Denoms(ctx, &types.QueryDenomsRequest{
		Pagination: &sdkquery.PageRequest{Key: firstPage.Pagination.NextKey, Limit: 1},
	})
	require.NoError(t, err)
	...
	_, err = k.Denoms(ctx, &types.QueryDenomsRequest{Pagination: &sdkquery.PageRequest{}})
	require.Error(t, err)
}
...
func TestKeeperDenomHashQuery(t *testing.T) {
	...
	traceResp, err := k.DenomHash(ctx, &types.QueryDenomHashRequest{Trace: "uatom"})
	require.NoError(t, err)
	require.Equal(t, types.DenomHash(native).String(), traceResp.Hash)

	traceResp, err = k.DenomHash(ctx, &types.QueryDenomHashRequest{Trace: types.DenomPath(traced)})
	require.NoError(t, err)
	require.Equal(t, types.DenomHash(traced).String(), traceResp.Hash)

	_, err = k.DenomHash(ctx, &types.QueryDenomHashRequest{Trace: ""})
	require.Error(t, err)
	require.Equal(t, codes.InvalidArgument, status.Code(err))

	_, err = k.DenomHash(ctx, &types.QueryDenomHashRequest{Trace: "transfer/channel-0/unknown"})
	require.Error(t, err)
	require.Equal(t, codes.NotFound, status.Code(err))
}
...
func TestKeeperEscrowAddressQuery(t *testing.T) {
	...
	t.Run("missing channel returns not found", func(t *testing.T) {
		_, err := k.EscrowAddress(ctx, &types.QueryEscrowAddressRequest{
			PortId:    "wrong-port",
	...
	t.Run("valid query returns escrow address", func(t *testing.T) {
		resp, err := k.EscrowAddress(ctx, &types.QueryEscrowAddressRequest{
			PortId:    types.PortID,
			ChannelId: "channel-0",
		})
		require.NoError(t, err)
...
func TestKeeperTotalEscrowForDenomQuery(t *testing.T) {
	...
	t.Run("missing denom returns zero", func(t *testing.T) {
		resp, err := k.TotalEscrowForDenom(ctx, &types.QueryTotalEscrowForDenomRequest{
			Denom: "missing",
		})
		require.NoError(t, err)
		require.Equal(t, "0", resp.Amount.Amount.String())
	...
	t.Run("invalid denom returns error", func(t *testing.T) {
		_, err := k.TotalEscrowForDenom(ctx, &types.QueryTotalEscrowForDenomRequest{
			Denom: "bad denom",
		})
		require.Error(t, err)
		require.Equal(t, codes.InvalidArgument, status.Code(err))
	...
	t.Run("found denom returns escrow", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, "123", resp.Amount.Amount.String())
