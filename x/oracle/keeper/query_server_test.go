...
func TestQueryParamsAndModerator(t *testing.T) {
	...
	moderator, err := k.ModeratorAddress(ctx, &types.QueryModeratorAddressRequest{})
	require.NoError(t, err)
...
func TestQueryRequestsAndReports(t *testing.T) {
	...
	respReq, err := k.OracleRequest(ctx, &types.QueryOracleRequestRequest{RequestId: 1})
	require.NoError(t, err)
	require.Equal(t, req, respReq.Request)

	respReqs, err := k.OracleRequests(ctx, &types.QueryOracleRequestsRequest{Status: types.Status_STATUS_ACTIVE})
	require.NoError(t, err)
	require.Len(t, respReqs.Requests, 1)

	respReports, err := k.OracleReports(ctx, &types.QueryOracleReportsRequest{
		RequestId: 1,
		Nonce:     2,
		Provider:  prov,
	})
...
func TestQueryResultsAndWhitelist(t *testing.T) {
	...
	resLatest, err := k.OracleResult(ctx, &types.QueryOracleResultRequest{RequestId: reqID})
	require.NoError(t, err)
	require.Equal(t, uint64(6), resLatest.Result.Nonce)

	resHistory, err := k.OracleResults(ctx, &types.QueryOracleResultsRequest{RequestId: reqID})
	require.NoError(t, err)
	...
	whitelist, err := k.Whitelist(ctx, &types.QueryWhitelistRequest{})
	require.NoError(t, err)
	require.Equal(t, uint64(1), whitelist.Count)
	require.Contains(t, whitelist.Addresses, addr)

	cats, err := k.Categories(ctx, &types.QueryCategoriesRequest{})
	require.NoError(t, err)
