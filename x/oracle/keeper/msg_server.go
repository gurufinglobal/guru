...
func (m MsgServer) validateModerator(ctx sdk.Context, moderator string) error {
	moderatorAddress, err := m.keeper.accountCodec.StringToBytes(moderator)
	if err != nil {
	...
	currentModeratorAddress, err := m.keeper.accountCodec.StringToBytes(currentModerator)
	if err != nil {
		return types.ErrInvalidAuthority.Wrapf("invalid configured moderator: %v", err)
	}
	if !bytes.Equal(currentModeratorAddress, moderatorAddress) {
		return types.ErrInvalidAuthority.Wrapf("invalid authority: expected %s, got %s", currentModerator, moderator)
	}
