...
// RunNativeAction prepares the native context to execute native action for stateful precompile,
...
func (p Precompile) runNativeAction(evm *vm.EVM, contract *vm.Contract, action NativeAction) (bz []byte, err error) {
	...
	// NOTE: guru's AddPrecompileFn takes (addr, snapshot, events)
...
// It returns the ABI method and calling arguments.
func SetupABI(
	api abi.ABI,
