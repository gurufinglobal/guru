...
func (b *Backend) GetTransactionReceipt(hash common.Hash) (map[string]interface{}, error) {
	...
	receipt := map[string]interface{}{
		...
		"contractAddress": nil,
		"gasUsed":         hexutil.Uint64(b.GetGasUsed(res, ethTx.GasPrice(), ethTx.Gas())),
	...
	// For dynamic fee transactions, compute effective gas price
	if ethTx.Type() == ethtypes.DynamicFeeTxType {
		...
		if err != nil {
			...
			effectiveGasPrice := new(big.Int).Add(ethTx.GasTipCap(), baseFee)
